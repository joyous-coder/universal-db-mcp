...
export class DatabaseService {
  ...
  async generateAndInsertSampleData(
    ...
    const columnsToInsert = options?.columns?.length
      ? options.columns
      : tableInfo.columns
          .filter(c => !autoExcludePks.includes(c.name.toLowerCase()))
          .map(c => c.name);
  ...
  async getTableInfo(tableName: string, forceRefresh: boolean = false): Promise<TableInfo> {
    ...
    // Adapter doesn't override getTableInfo — fall back to getSchema scan
    // (rare path; mostly Redis/MongoDB which don't expose per-table meta).
