            ...
            properties: {
              ...
              options: {
                ...
                properties: {
                  ...
                  rules: {
                    type: 'array',
                    description: '生成规则数组。每条 rule: { match: { columnName? | columnNamePattern? | tableName? | columnType? }, generate: { type, ... } }',
                    items: {
                      type: 'object',
                      description: '单条生成规则',
                      properties: {
                        ...
                        generate: {
                          type: 'object',
                          description: '生成策略(9 种 type,见下方 examples)',
                          properties: {
                            type: {
                              type: 'string',
                            ...
                            value: { description: 'fixed: 固定值' },
                            min: { type: 'number', description: 'range: 最小值' },
                            max: { type: 'number', description: 'range: 最大值' },
                            decimals: { type: 'number', description: 'range: 小数位' },
                            template: { type: 'string', description: 'pattern: 模板字符串 (支持占位符)' },
                            method: { type: 'string', description: 'faker: 方法名 (如 internet.email)' },
                            args: { description: 'faker: 参数数组' },
                            values: { type: 'array', items: { type: 'string' }, description: 'choice: 候选取值' },
                            start: { type: 'number', description: 'sequence: 起始值' },
                            step: { type: 'number', description: 'sequence: 步长' },
                            format: { type: 'string', description: 'sequence: 格式如 05d' },
                            pattern: { type: 'string', description: 'regex: 正则模式' },
                          },
                          required: ['type'],
                        },
                      },
                    ...
                    examples: [
                      { type: 'fixed', value: 'BBZ_PROVINCE_EG', _exampleFor: '固定值 tenant_id' },
                      { type: 'range', min: 100, max: 10000, decimals: 2, _exampleFor: '数值范围 amount' },
                      { type: 'pattern', template: 'PRJ-{year}-{sequence:05d}', _exampleFor: '模板 project_code' },
                      { type: 'faker', method: 'internet.email', _exampleFor: 'faker email' },
                      { type: 'choice', values: ['pending', 'paid', 'shipped'], _exampleFor: '从列表 status' },
                      { type: 'sequence', start: 1, step: 1, format: '05d', _exampleFor: '自增序号 id' },
                      { type: 'regex', pattern: '^[A-Z]{3}-\\d{4}$', _exampleFor: '正则 code' },
                      { type: 'null', _exampleFor: '总是 NULL' },
                      { type: 'skip', _exampleFor: '用 DB default' },
                    ],
                  },
