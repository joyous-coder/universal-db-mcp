...
- **Bug #15 (DM adapter)**: `get_table_info` 在达梦上失败,提示"表 X 不存在"。
  两个根因同时修:
  - 索引列名错: `ALL_IND_COLUMNS` join 用 `ic.IND_OWNER` → 修正为 `ic.INDEX_OWNER`
    (与 Oracle 字典列名一致,DM 兼容)。
  - 裸名表无 owner: 当用户传 `T_USERS` (无 schema. 前缀) 时,owner 空字符串 → fallback
    `owner = this.config.user` (DM 默认 user = schema)。
- **Bug #17 (generate_sample_data)**: 含 IDENTITY/UUID/INT 主键时绑值冲突。
  完整修复:
  - DM `IDENTITY(1,1)` 列被 bind 整数 → 协议层报"表或视图不存在"。修复:adapter 检测
    IDENTITY (`ALL_TAB_IDENTITY_COLS`)、generator 跳过该列(DB 自填)。
  - Oracle `GENERATED BY DEFAULT AS IDENTITY` 同上,补全 `ALL_TAB_IDENTITY_COLS` 查询。
  - 非 IDENTITY INT PK 列 (用户维护序列):之前生成 1-100000 随机 int,可能冲突。
    修复:生成前查 `SELECT COALESCE(MAX(pk), 0)`,从 `max + rowIndex + 1` 开始 (databaseService
    注入 `context.maxIntPkValues`)。
  - VARCHAR2(36) UUID PK: 之前排除该列 → DB 默认 NULL → ORA-01400。修复:generator 给
    `CHAR(36)/VARCHAR2(36)/VARCHAR(36)` 类型生成 uuid v4。
  - DM `dmdb` driver 不接受 JS Date 对象 bind timestamp 列 → 改为 ISO 字符串。
  - `fallbackByType` regex 加 `number`,匹配 Oracle/DM/PostgreSQL 通用 `NUMBER` 类型
    (之前 NUMBER 漏掉 → fallback lorem sentence → bind NUMBER 列 ORA-01722)。

### ⚡ 性能优化

- **`get_table_info` per-table 路径** (Oracle/DM): 由 getSchema 全表扫描
  (60-90s) 改为 4 SQL 单表查询 (~600-900ms, 50-100 倍提升)。
  新增 `DbAdapter.getTableInfo?(tableName)` 可选方法,Oracle + DM + SQLite + ClickHouse 实现。
- **移除 per-table cache** (v4.0.3.1): 866ms 冷启动已可接受,cache 增复杂度且有 stale 风险。

### 🔧 改动文件

- `src/adapters/oracle.ts` — getTableInfo fast path + ALL_TAB_IDENTITY_COLS 标记 IDENTITY
- `src/adapters/dm.ts` — getTableInfo INDEX_OWNER 修正 + owner fallback + IDENTITY 查询
- `src/adapters/base.ts` — 默认 getTableInfo fallback to getSchema
- `src/adapters/sqlite/index.ts`, `src/adapters/clickhouse.ts` — 公开 private getTableInfo
- `src/types/adapter.ts` — `getTableInfo?` optional 方法
- `src/core/database-service.ts` — per-table adapter-direct path + MAX(pk) 注入
- `src/utils/sample-data-generator.ts` — PK 智能生成 (IDENTITY 跳过 / UUID / MAX+rowIndex)

### 🧪 测试报告

- Oracle 17c: `docs/04-reports/2026-08-17-oracle-tool-validation.md`
- DM 8: `docs/04-reports/2026-08-18-dm-tool-validation.md`

### 兼容说明

- API 兼容,无需迁移用户配置。
- 唯一行为变更: `generate_sample_data` 现在智能生成 PK 值 (UUID/MAX+1),不再需要手动排除 IDENTITY 列。
...
#### Server 端 listChanged 通知改善 (`src/mcp/mcp-server.ts:273-280`)

- **之前**: `use_tool_group` 调 `server.sendToolListChanged()`,但用 try/catch **静默吞掉** 任何错误,运维无法察觉通知发送失败。
- **现在**: 同样的 try/catch,但 **catch 块打印 `console.error`**,SDK 异常有日志可见。

#### Tool description 升级 (`src/mcp/{mcp-server,tool-definitions}.ts`)

`use_tool_group` 描述现在明确告知:
> 激活后服务端会按 MCP 协议发 `notifications/tools/list_changed`;**若客户端不消费该通知(已知 Claude Code 当前实现),需要重启客户端或在 MCP 客户端设置中手动刷新**。

`use_tool_schema` 描述明确告知:
> 不影响工具列表,不需要刷新客户端。

#### 新增测试 (`tests/unit/lazy-loading-notification.test.ts`)

5 项新增 unit test:
- SDK `sendToolListChanged` 真的发出 `notifications/tools/list_changed` JSON-RPC 帧
- SDK capability 声明 `listChanged: true` 不抛错
- `ToolRegistry.activateGroup` 行为正确(`newlyAvailable` 增量返回)
- `listActiveTools` 反映 per-session 激活
- tool definitions 文案断言包含 `notifications/tools/list_changed` + `Claude Code` + `重启/刷新`

#### 文档 (`docs/03-features/lazy-loading.md`)

新增 **⚠️ Claude Code 客户端限制** 章节:
- 列出 7+ MCP 客户端的 listChanged 支持情况
- 解释 Claude Code 行为根因(客户端缓存,非服务端问题)
- 提供 Claude Code 用户的 workaround(`DB_LAZY_DEFAULT_GROUP=query-experience` opt-in)
- 给出按客户端的推荐用法对照表

### 用户影响

- **Cline / Continue / Dify / Cherry Studio / 5ire 等**:lazy loading 行为现在可验证 — 服务端确实发通知,日志可查
- **Claude Code**: 行为不变(必须重启才能看到新激活的 tool),但 tool description + 文档已告知用户正确做法
- **所有用户**: 测试覆盖 + 错误日志更可观测

### 版本号规则

Patch release(3.3.0 → 3.3.1): bugfix-only, 无新 API,无 breaking changes。
